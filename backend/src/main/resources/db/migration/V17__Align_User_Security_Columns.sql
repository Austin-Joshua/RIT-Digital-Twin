...
SET @sql = IF(@col_exists = 0, 'ALTER TABLE users ADD COLUMN account_status VARCHAR(20) NOT NULL DEFAULT "active"', 'SELECT 1');
...
SET @sql = IF(@col_exists = 0, 'ALTER TABLE users ADD COLUMN must_change_password BOOLEAN DEFAULT FALSE', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0, 'ALTER TABLE users ADD COLUMN failed_login_attempts INT DEFAULT 0', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0, 'ALTER TABLE users ADD COLUMN lock_until DATETIME DEFAULT NULL', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0, 'ALTER TABLE users ADD COLUMN last_password_change DATETIME DEFAULT NULL', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
